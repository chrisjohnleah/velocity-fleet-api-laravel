...
/**
 * A vehicle breached a geofence boundary (entered or exited). The {@see Geofence}
 * is supplied by the geofences module and referenced here by FQN.
 */
final class GeofenceBreachedNotification extends Notification
{
    ...
    public function __construct(
        ...
        public readonly string $direction = 'entered',
    ) {
    }
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        $verb = $this->directionVerb();

        return (new MailMessage())
            ->subject("Geofence breach: {$vehicle}")
            ->line("{$vehicle} has {$verb} {$place}.")
            ->line($this->whenLine());
    }
    ...
    private function directionVerb(): string
    {
        return $this->direction === 'exited' ? 'left' : 'entered';
    }
    ...
    private function whenLine(): string
    {
        ...
        return $iso === null ? 'Time was not reported.' : "Time: {$iso}.";
    }
